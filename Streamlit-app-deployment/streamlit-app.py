...
st.write("Enter the following parameters/choices to predict house price")
...
living_area = st.slider("living area", min_value=370, max_value=13540)
...
waterfront_present = st.slider("Waterfront present (0 or 1)", min_value=0, max_value=1, step=1)
...
Lattitude = st.slider("Enter the latitude", min_value=52.3859, max_value=53.0076, step=0.1)
...
if st.button("Submit"):
    try:
        ...
        # Ensure the prediction is displayed as a float
